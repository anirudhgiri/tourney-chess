...
tournamentRouter.get("/:id", async (req, res) => {
	...
	else res.json({ success: true, message: tournament });
});
...
tournamentRouter.post("/", async (req, res) => {
	...
	let user = await User.findOneAndUpdate(
		...
		{ $push: { tournaments: tournament.privateId } }
	);
...
tournamentRouter.put("/:privateId/makePublic", async (req, res) => {
	...
	if (!tournament)
		...
		res.status(400).json({
			success: false,
			message: "Tournament is already public",
		});
	else if (req.session.userId == tournament.owner) {
		tournamentModel
			.findOneAndUpdate({ id: req.params.privateId }, { isPublic: true })
			.exec();
		res.status(200).json({
			success: true,
			message: "Tournament successfully made public!",
		});
	} else
		res.status(403).json({
			success: false,
...
tournamentRouter.put("/:privateId/makePrivate", async (req, res) => {
	...
	if (!tournament)
		...
		res.status(400).json({
			success: false,
			message: "Tournament is already private",
		});
	else if (req.session.userId == tournament.owner) {
		tournamentModel
			.findOneAndUpdate({ id: req.params.privateId }, { isPublic: false })
			.exec();
		res.status(200).json({
			success: true,
			message: "Tournament successfully made private!",
		});
	} else
		res.status(403).json({
			success: false,
...
tournamentRouter.delete("/:privateId", async (req, res) => {
	...
	if (!tournament)
		...
		User.findOneAndUpdate(
			...
			{ $pull: { tournaments: req.params.privateId } }
		).exec();
		res.status(200).json({
			success: true,
...
tournamentRouter.use("/:privateId/players", playerRouter);
