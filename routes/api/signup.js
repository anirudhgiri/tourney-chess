...
const { signUpSchema } = require("../../utils/validation/schemas/signup");
const {
	validationMiddleware,
} = require("../../utils/validation/validationMiddleware");

signupRouter.post("/", validationMiddleware(signUpSchema), async (req, res) => {
	...
	if (userlookup)
		...
		const passwordHash = crypto
			.pbkdf2Sync(req.body.password, passwordSalt, 100000, 64, "sha512")
			.toString("hex");
